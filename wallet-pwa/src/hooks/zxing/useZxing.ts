...
import { DecodeContinuouslyCallback, DecodeHintType, Exception, Result } from '@zxing/library';
...
export const useZxing = (
  ...
  const startDecoding = useCallback(async () => {
    ...
    if (reader.isVideoPlaying(ref.current)) {
      console.warn('video is already playing: avoiding multiple instances running');
      return;
    }
