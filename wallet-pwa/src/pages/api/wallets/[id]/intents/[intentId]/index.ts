...
import { getWalletPaymentIntent } from '@paggo/services/services';
import { HttpMethod } from '@paggo/types';

import type { NextApiResponse } from 'next';

async function get(req: NextApiRequestLogged, res: NextApiResponse<any>) {
  const intent = await getWalletPaymentIntent(req.query.intentId as string, req.id, req.user);

  return res.status(HttpStatusCode.Ok).send(jsonBigInt(intent));
}
